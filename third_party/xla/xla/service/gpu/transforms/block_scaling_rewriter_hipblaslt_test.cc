...
ENTRY main {
  ...
  EXPECT_TRUE(RunAndCompare(
      ...
      [&](HloModule* reference_module) {
        BlockScalingRewriter pass(this->device_desc(), /*allow_hipblaslt=*/false);
        EXPECT_THAT(RunHloPass(&pass, reference_module), IsOkAndHolds(true));
      },
      /*test_preprocessor=*/
      [&](HloModule* test_module) {
        BlockScalingRewriter pass(this->device_desc(), /*allow_hipblaslt=*/true);
        EXPECT_THAT(RunHloPass(&pass, test_module), IsOkAndHolds(true));
      }));

  RunAndFilecheckHloRewrite(
      hlo_string,
      BlockScalingRewriter(this->device_desc(), /*allow_hipblaslt=*/false),
      "CHECK-NOT: __cublas$lt$matmul$mx");
  RunAndFilecheckHloRewrite(
      hlo_string,
      BlockScalingRewriter(this->device_desc(), /*allow_hipblaslt=*/true),
