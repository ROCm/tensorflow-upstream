...
struct StridedGemmConfig {
  ...
  int64 lda, ldb, ldc;
  int64 stride_a, stride_b, stride_c;
};
...
struct BlasLtGemmRunner {
  ...
  xla::Status Run(Stream& stream, blas::Transpose trans_a,
      ...
      Scalar alpha, const DeviceMemory<TypeA>& a, int64 lda,
      const DeviceMemory<TypeB>& b, int64 ldb,
      Scalar beta, DeviceMemory<TypeC> *c, int64 ldc,
  ...
  xla::Status Run(Stream& stream, blas::Transpose trans_a,
      blas::Transpose trans_b, int64 m, int64 n, int64 k,
      Scalar alpha, const TypeA* a, int64 lda,
      const TypeB *b, int64 ldb,
      Scalar beta, TypeC *c, int64 ldc,
      ScratchAllocator* allocator) {
    ...
    return RunStridedBatchedImpl(stream, trans_a, trans_b, m, n, k,
        Convert(alpha),
        type_a, DeviceMemoryBase{const_cast< TypeA *>(a)}, lda, 0,
        type_b, DeviceMemoryBase{const_cast< TypeB *>(b)}, ldb, 0,
        Convert(beta).real(), // only real betas are supported!!
  ...
  xla::Status RunStridedBatched(Stream& stream, blas::Transpose trans_a,
      blas::Transpose trans_b, int64 m, int64 n, int64 k,
      Scalar alpha, const TypeA* a, int64 lda, int64 stride_a,
      const TypeB* b, int64 ldb, int64 stride_b,
  ...
  xla::Status RunStridedBatched(Stream& stream, blas::Transpose trans_a,
      blas::Transpose trans_b, int64 m, int64 n, int64 k,
      Scalar alpha, const DeviceMemory<TypeA>& a, int64 lda, int64 stride_a,
    ...
    return RunStridedBatchedImpl(stream, trans_a, trans_b, m, n, k,
        Convert(alpha),
        type_a, a, lda, stride_a, type_b, b, ldb, stride_b,
        Convert(beta).real(), // only real betas are supported!!
  ...
  xla::Status RunBatched(Stream& stream, blas::Transpose trans_a,
        ...
        const ArraySlice<DeviceMemory<T> *> &a, int64 lda,
        const ArraySlice<DeviceMemory<T> *> &b, int64 ldb, Scalar beta,
        const ArraySlice<DeviceMemory<T> *> &c, int64 ldc,
        int64 batch_count, ScratchAllocator* allocator) {

  // NOTE: Scalar types shall be verified for correctness vs T!!
    auto type = dnn::ToDataType<T>::value;
    auto cvt = [](auto x){
      using TT = ArraySlice<DeviceMemoryBase *>;
      auto ptr = reinterpret_cast<const void *>(&x);
      return *reinterpret_cast<const TT*>(ptr);
    };
    ...
    return xla::InternalError("RunBatched: port::ArraySlice NYI!");
  }


  template < class Scalar, class T >
  xla::Status RunBatched(Stream& stream, blas::Transpose trans_a,
      blas::Transpose trans_b, uint64 m, uint64 n, uint64 k,
      Scalar alpha, const T** a, int lda,
      const T** b, int ldb, Scalar beta,
