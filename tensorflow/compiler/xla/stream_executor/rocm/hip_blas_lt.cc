...
static_assert(sizeof(hipblasLtMatmulAlgo_t) == sizeof(rocblaslt_matmul_algo),
    "Structure size does not match!");
...
BlasLt::BlasLt(gpu::GpuExecutor* parent)
...
auto BlasLt::MatmulPlan::GetAlgorithms(size_t max_algorithm_count,
  ...
  for (const hipblasLtMatmulHeuristicResult_t& result : results) {
    if (result.state == HIPBLAS_STATUS_SUCCESS) {  // Skip failed algos.
      auto roc_algo = (const rocblaslt_matmul_algo*)&result.algo;
      auto pindex = (int *)roc_algo->data;
      algorithms.push_back({result.algo, result.workspaceSize, 
            static_cast< blas::AlgorithmType >(*pindex)});
...
auto BlasLt::GetMatmulPlan(const gpu::GemmConfig& cfg) const
  ...
  TF_ASSIGN_OR_RETURN(auto compute_type, gpu::GetBlasComputationType(
...
xla::Status BlasLt::MatmulPlan::DoMatmul(
  ...
  if (profile_result != nullptr) {
    ...
    profile_result->set_algorithm(algorithm_->id);
...
// gpu::ScopedActivateExecutorContext sac{blas_lt_ref_.parent_}; ??
  ...
  for(auto& res : heuristicResult) {
    ...
    if(grouped_gemm_->isAlgoSupported(res.algo, workspace_size)) {
      auto roc_algo = (const rocblaslt_matmul_algo*)&res.algo;
      auto pindex = (int *)roc_algo->data;
