...
#include "tensorflow/compiler/xla/stream_executor/gpu_asm_opts.h"
...
#include "tensorflow/compiler/xla/stream_executor/stream_executor_memory_allocator.h"
...
static void EXPECT_REDZONE_VIOLATION(
    StatusOr<RedzoneCheckStatus> status) {
  EXPECT_TRUE(status.ok());
...
TEST(RedzoneAllocatorTest, WriteToRedzone) {
  ...
  Platform* platform =
      PlatformManager::PlatformWithName(tensorflow::GpuPlatformName()).value();
  ...
  auto modify_redzone = [&](DeviceMemoryBase redzone, int64_t offset,
    ...
    { EXPECT_REDZONE_OK(allocator.CheckRedzones()); }
    TF_ASSERT_OK(stream->ThenMemcpy(&old_redzone_value, redzone_at_offset, 1));
...
TEST(RedzoneAllocatorTest, VeryLargeRedzone) {
  ...
  Platform* platform =
      PlatformWithName(tensorflow::GpuPlatformName()).value();
