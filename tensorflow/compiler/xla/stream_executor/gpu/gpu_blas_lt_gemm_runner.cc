...
std::ostream& operator<<(std::ostream& os, const StridedGemmConfig& cfg) {
  return os << "trans_a/b: " << (int)cfg.trans_a << "/" << (int)cfg.trans_b
            ...
            << " batch_count: " << cfg.batch_count << " lda: " << cfg.lda
            << " ldb: " << cfg.ldb << " ldc: " << cfg.ldc
            << " stride_a: " << cfg.stride_a << " stride_b: " << cfg.stride_b
            << " stride_c: " << cfg.stride_c << " type_a: " << (int)cfg.type_a
            << " type_b: " << (int)cfg.type_b << " type_c: " << (int)cfg.type_c
...
/*static*/ BlasLtGemmRunner& BlasLtGemmRunner::i(const Stream* stream) {
  ...
  if (!res) {
    autotune_enabled_ =
        xla::GetDebugOptionsFromFlags().xla_gpu_autotune_level() > 0;
    res.reset(new BlasLtGemmRunner(stream->parent()));
    xla::gpu::AutotunerUtil::LoadAutotuneResultsFromFileOnce(
        *res->autotune_config_);
  }
  return *res;
}
  ...
  for (uint32_t j = 0; j < algorithms.size(); j++) {
    ...
    for (i = 0, total_ms = 0; i < n_total; i++) {
      ...
      if (!res.ok() || !profile.is_valid()) {
        VLOG(1) << j
                << ": gemm algorithm is not valid: " /* << res.error_message() */;
        break;
      }
...
xla::StatusOr<std::array<uint64_t, 3>> BlasLtGemmRunner::ContiguousStrides(
    ...
    const ArraySlice<DeviceMemoryBase*>& c, int64_t batch_count) {
...
xla::Status BlasLtGemmRunner::RunBatchedImpl(
    ...
    const void** a, int64_t lda, blas::DataType type_b, const void** b,
    int64_t ldb, const void* beta, blas::DataType type_c, void** c, int64_t ldc,
  ...
  if (res == grouped_gemm_map_.end()) {
    ...
    TF_ASSIGN_OR_RETURN(
        auto algorithms,
        res->second->GetAlgorithms(num_solutions,
                                   allocator == nullptr ? 0 : 1ull << 32));

    VLOG(1) << stream.parent()
            << ": new GGemm config: " << grouped_gemm_map_.size()
            << " #valid algorithms: " << algorithms.size();

    BlasLt::MatmulAlgorithm best_algo;
    if (!autotune_enabled_) {
      if (algorithms.empty())
        return xla::InternalError("No GG algorithms found!");
      best_algo = algorithms[0];  // otherwise use default algorithm
    } else {
      TF_ASSIGN_OR_RETURN(
          auto best_algo,
          Autotune(algorithms, [&](const gpu::BlasLt::MatmulAlgorithm& algo,
                                   blas::ProfileResult* profile) {
            if (profile == nullptr) {
...
xla::Status BlasLtGemmRunner::RunStridedBatchedImpl(
    Stream& stream, blas::Transpose trans_a, blas::Transpose trans_b, int64_t m,
    int64_t n, int64_t k, xla::complex128 alpha, blas::DataType type_a,
    const DeviceMemoryBase& a, int64_t lda, int64_t stride_a,
    blas::DataType type_b, const DeviceMemoryBase& b, int64_t ldb,
    int64_t stride_b, double beta, blas::DataType type_c, DeviceMemoryBase* c,
  ...
  while (res == strided_gemm_map_.end()) {
    ...
    GemmConfig cfg = {
        ...
        .c_layout =
            MatrixLayout(type_c, m, n, order, batch_count, ldc, stride_c),
        .output_layout =
            MatrixLayout(type_c, m, n, order, batch_count, ldc, stride_c),
        .alpha = alpha,
        .beta = beta,
    ...
    VLOG(1) << &stream << " dev " << stream.parent() << '('
            ...
            << "): new StridedBatched config: " << strided_gemm_map_.size()
            << " #algorithms: " << algorithms.size();

    if (!autotune_enabled_) {
      if (algorithms.empty())
        return xla::InternalError("No algorithms found!");
    ...
    if (best_algo.id == blas::kNoAlgorithm) {
      TF_ASSIGN_OR_RETURN(
          ...
          Autotune(algorithms, [&](const gpu::BlasLt::MatmulAlgorithm& algo,
            ...
            return res->second->ExecuteOnStream(&stream, a, b, *c, *c,
      ...
      auto new_id = xla::gpu::AutotunerUtil::AddResultToInMemoryCache(
