...
struct BlasLtGemmRunner {
  ...
  xla::Status RunBatched(
      Stream& stream, blas::Transpose trans_a, blas::Transpose trans_b,
      int64_t m, int64_t n, int64_t k, Scalar alpha,
      const ArraySlice<DeviceMemory<T>*>& a, int64_t lda,
      const ArraySlice<DeviceMemory<T>*>& b, int64_t ldb, Scalar beta,
      const ArraySlice<DeviceMemory<T>*>& c, int64_t ldc,
    ...
    return xla::InternalError("RunBatched: ArraySlice NYI!");
  }

  template <class Scalar, class T>
  xla::Status RunBatched(Stream& stream, blas::Transpose trans_a,
                         blas::Transpose trans_b, uint64 m, uint64 n, uint64 k,
                         Scalar alpha, const T** a, int lda, const T** b,
                         int ldb, Scalar beta, T** c, int64_t ldc,
                         int64_t batch_count, ScratchAllocator* allocator) {
    auto type = dnn::ToDataType<T>::value;
 ...
 private:
  ...
  xla::Status RunBatchedImpl(Stream& stream, blas::Transpose trans_a,
                             ...
                             blas::DataType type_a, const void** a, int64_t lda,
                             blas::DataType type_b, const void** b, int64_t ldb,
                             const void* beta, blas::DataType type_c, void** c,
                             int64_t ldc, int64_t batch_count,
                             ScratchAllocator* allocator);

  xla::Status RunStridedBatchedImpl(
      Stream& stream, blas::Transpose trans_a, blas::Transpose trans_b,
  ...
  absl::flat_hash_map<GroupedGemmConfig, BlasLt::GroupedMatmulPlanPtr>
      grouped_gemm_map_;
  absl::flat_hash_map<StridedGemmConfig, BlasLt::MatmulPlanPtr>
      strided_gemm_map_;
};
