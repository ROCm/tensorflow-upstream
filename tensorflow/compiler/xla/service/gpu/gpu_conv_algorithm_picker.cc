  ...
  switch (kind) {
    ...
    case se::dnn::ConvolutionKind::BACKWARD_FILTER: {
      ...
      TF_RETURN_IF_ERROR(stream_exec->GetConvolveRunners(
          ...
          se::dnn::CallContext::kNone, use_fallback, nullptr, se::NumericOptions{deterministic_ops},
          &runners));
      for (auto& runner : runners) {
        TF_ASSIGN_OR_RETURN(
            auto runner_cache,
...
GetMIOpenAlgorithms(const HloCustomCallInstruction* instr,
                    ...
                    ScratchAllocator* scratch_allocator, se::dnn::CallContext call_context, se::Stream* stream,
...
auto& autotune_cache ABSL_GUARDED_BY(autotune_cache_mu) =
    *new absl::flat_hash_map<AutotuneConvCacheKey, AutotuneResult>();
...
StatusOr<AutotuneResult> GpuConvAlgorithmPicker::PickBestAlgorithm(
  ...
  auto stream_exec = config_.GetExecutor();
  ...
  // models).
  AutotuneConvCacheKey key = AutotunerUtil::ConvCacheKeyFromInstruction(
...
GpuConvAlgorithmPicker::AutotuneRuntimeArguments::FromInstruction(
  ...
  // Get canonical HLO.
  std::string canonical_hlo = std::get<1>(AutotunerUtil::ConvCacheKeyFromInstruction(
      instr, stream_exec->GetDeviceDescription().model_str()));
...
GpuConvAlgorithmPicker::PickBestAlgorithmNoCacheRocm(
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      GetMIOpenAlgorithms(instr, absl::MakeSpan(operand_buffers), result_buffer,
                          stream_exec, &scratch_allocator, call_context, stream,
                          deterministic_ops));
