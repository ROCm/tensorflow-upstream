...
Type TensorFlowRefType::RemoveRef() {
  ...
  if (mlir::isa<Float8E4M3FNType>(*this)) return Float8E4M3FNType::get(ctx);
  if (mlir::isa<Float8E5M2Type>(*this)) return Float8E5M2Type::get(ctx);
  if (mlir::isa<Float8E4M3FNUZType>(*this)) return Float8E4M3FNUZType::get(ctx);
  if (mlir::isa<Float8E4M3B11FNUZType>(*this))
    return Float8E4M3B11FNUZType::get(ctx);
  if (mlir::isa<Float8E5M2FNUZType>(*this)) return Float8E5M2FNUZType::get(ctx);
