...
#include "ck_tile/03_gemm_ln_attn/gemm_ln_attn.hpp"
#include "op.h"
...
Status ComputeInternal<ck_tile::fp16_t>(const GPUDevice& d, const void* mat_A0,
  ...
  using gemm_ln_attn_pipeline_problem_0 =
      ck_tile::BlockGemmLnAttnPipelineProblem<
          ...
          typename GemmLnAttnTypeConfig<
              gemm_ln_attn_dtype_0>::SMPLComputeDataType,
  ...
  using gemm_ln_attn_epilogue_0 =
      ck_tile::Default2DEpilogue<ck_tile::Default2DEpilogueProblem<
          ...
          typename GemmLnAttnTypeConfig<ck_tile::fp16_t>::ODataType, true,
          true>>;
  ...
  auto gemm_ln_attn_args_device = gemm_ln_attn_args{
      mat_A0,        mat_B0,       mat_C, mat_B1,   mat_B2,      Gamma,
      Beta,          Keymask,      mat_D, K,        M,           N0,
      long_seq,      N2,           B_kv,  head_num, lrelu_alpha, do_layer_norm,
      do_leaky_relu, do_query_mask};
...
namespace functor {
template <typename T>
struct GemmLayernormGemmSoftmaxGemmFunctor<GPUDevice, T> {
 public:
  static Status Compute(const GPUDevice& d, const void* mat_A0,
    ...
    if constexpr (std::is_same_v<T, Eigen::half>) {
      return ComputeInternal<ck_tile::fp16_t>(
          d, mat_A0, mat_B0, mat_C, mat_B1, mat_B2, Gamma, Beta, Keymask, mat_D,
          K, M, N0, N1, long_seq, N2, B_kv, head_num, lrelu_alpha,
          do_layer_norm, do_leaky_relu, do_query_mask);
    }
