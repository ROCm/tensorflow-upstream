...
class GatherGemv2Test : public OpsTestBase {
 protected:
    ...
    for (int i = 0; i < index; i++) {
        for (int s = 0 ; s < seq; s++){
          for (int k = 0; k < head_sz * head_num; k++){
            int tmp = indices[i];
            new_mat_B0.push_back(mat_B0[tmp * seq * head_sz * head_num + s * head_sz * head_num + k]);
          }
        }
    }
        ...
        for(int b = 0; b < index; b++){
            for(int k = 0; k < head_sz; k++){
                ...
                for(int s = 0; s < seq; s++){
                    ...
                    float breg = float(new_mat_B0[head_num * b * seq * head_sz + head_num * s * head_sz + h * head_sz + k]);
                    psum += areg * breg;
                }
  ...
  void RunGatherGemv2Test(const std::vector<T>& mat_A,
    ...
    Tensor expected(allocator(), dt, TensorShape({index , 1, head_num * head_sz}));
...
TEST_F(GatherGemv2Test, Simple3) {
    ...
    RunUnfusedGatherGemv2Test<Eigen::half>(mat_A, mat_B, indices, mat_D, head_sz, seq, b, index, head_num);
    RunGatherGemv2Test<Eigen::half>(mat_A, mat_B, indices, mat_D, head_sz, seq, b, index, head_num);
}
