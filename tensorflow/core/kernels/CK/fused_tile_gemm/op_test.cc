...
namespace {
    ...
    protected:
        ...
        void RunFusedTileGemmTest(const std::vector<Eigen::half>& mat_A,
            ...
            VLOG(2) << "Expected output tensor (first few values):" << std::endl;
            for (int i = 0; i < std::min(M * N, 10); ++i) {
                VLOG(2) << float(expected.flat<Eigen::half>()(i)) << " ";
            }
            VLOG(2) << std::endl;
            ...
            VLOG(2) << "Actual output tensor (first few values):" << std::endl;
            for (int i = 0; i < std::min(M * N, 10); ++i) {
                VLOG(2) << float(output_flat(i)) << " ";
            }
            VLOG(2) << std::endl;

            for (int i = 0; i < M * N; ++i) {
                if (abs(float(expected_flat(i)) - float(output_flat(i))) > abs_err) {
                    VLOG(2) << "Mismatch at index " << i << ": expected " 
            ...
            if (all_close) {
                VLOG(INFO) << "All values are within tolerance." << std::endl;
            } else {
                LOG(ERROR) << "There are mismatched values." << std::endl;
            }
    ...
    TEST_F(FusedTileGemmTest, MediumMatrices) {
        ...
        VLOG(2) << "Initial values of matrix A (first few values):" << std::endl;
        for (int i = 0; i < std::min(M * K, 10); ++i) {
            VLOG(2) << float(mat_A[i]) << " ";
        }
        VLOG(2) << std::endl;

        VLOG(2) << "Initial values of matrix B (first few values):" << std::endl;
        for (int i = 0; i < std::min(K * N, 10); ++i) {
            VLOG(2) << float(mat_B[i]) << " ";
        }
        VLOG(2) << std::endl;
        ...
        VLOG(2) << "Reference result (first few values):" << std::endl;
        for (int i = 0; i < std::min(M * N, 10); ++i) {
            VLOG(2) << float(mat_C[i]) << " ";
        }
        VLOG(2) << std::endl;
