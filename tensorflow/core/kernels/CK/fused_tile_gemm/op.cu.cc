...
public:
    static Status Compute(const GPUDevice&d, int M, int N, int K, int Batch, const void* a0, const void* b0, const void* d0, void* e) {
        ...
        auto argument =
            device_op.MakeArgument(a0,
                                b0,
                                std::array<const void*, NumDTensor>{d0},
                                e,
                                M,
                                N,
                                K,
                                K,
                                K,
                                std::array<ck::index_t, NumDTensor>{ck::Number<0>{}},
                                N,
                                AElementOp{},
                                BElementOp{},
                                CDEElementOp{});
        ...
        {
            return errors::InvalidArgument("wrong! device_gemm with the specified compilation parameters does "
                "not support this GEMM problem");
        }

        float ave_time = invoker.Run(argument, StreamConfig{stream, time_kernel, 20, 50});
        if(time_kernel){
            ...
            LOG(INFO) << "Running time: " << ave_time << " ms, " << tflops << " TFlops, " << gb_per_sec << " GB/s";
        }
        return Status::OK();
    }
