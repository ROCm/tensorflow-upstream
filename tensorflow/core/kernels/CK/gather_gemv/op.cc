...
class GatherGemvOp : public OpKernel {
 public:
  explicit GatherGemvOp(OpKernelConstruction* context) : OpKernel(context) {
    OP_REQUIRES_OK(context, context->GetAttr("head_num", &head_num_));
  }

  void Compute(OpKernelContext* context) override {
    int head_sz = context->input(0).shape().dim_size(2) / head_num_;
    ...
    OP_REQUIRES_OK(context, context->allocate_output(0, {index, head_num_, seq},
                                                     &output_tensor));

    OP_REQUIRES_OK(
        context,
        functor::GatherGemvFunctor<Device, dataTP>::Compute(
