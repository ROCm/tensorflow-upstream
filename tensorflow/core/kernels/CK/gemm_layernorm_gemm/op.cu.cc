...
#include "ck_tile/04_gemm_ln_gemm/gemm_ln_gemm.hpp"
#include "op.h"
...
Status ComputeInternal(const GPUDevice& d, const void* mat_A0,
  ...
  using gemm_ln_gemm_pipeline_problem_0 =
      ck_tile::BlockGemmLnGemmPipelineProblem<
  ...
  using gemm_ln_gemm_epilogue_0 =
      ck_tile::Default2DEpilogue<ck_tile::Default2DEpilogueProblem<
          ...
          typename GemmLnGemmTypeConfig<ck_tile::fp16_t>::ODataType, true,
          true>>;
  ...
  static Status Compute(const GPUDevice& d, const void* mat_A0,
    ...
    if constexpr (std::is_same_v<T, Eigen::half>) {
      return ComputeInternal<ck_tile::fp16_t>(
          d, mat_A0, mat_B0, mat_C, mat_B1, Gamma, Beta, mat_D, K, M, N0, N1,
          head_num, lrelu_alpha, do_layer_norm, do_leaky_relu);
    }
