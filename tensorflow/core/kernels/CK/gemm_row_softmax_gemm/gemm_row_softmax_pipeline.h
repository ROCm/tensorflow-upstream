...
struct BlockGemmRowSoftmaxPipeline
{
    ...
    using GemmRowSoftmaxTileShape = remove_cvref_t<typename Problem::GemmRowSoftmaxTileShape>;
    ...
    template <typename B0TDramBlockWindowTmp,
              typename A0DramBlockWindowTmp,
              typename MaskDramBlockWindowTmp,
              typename B1DramBlockWindowTmp>
    CK_TILE_HOST_DEVICE auto operator()(const B0TDramBlockWindowTmp& b0t_dram_block_window_tmp,
                                        const A0DramBlockWindowTmp& a0_dram_block_window_tmp,
                                        const MaskDramBlockWindowTmp& mask_dram_block_window_tmp,
                                        B1DramBlockWindowTmp& b1_dram_block_window_tmp,
                                        index_t seqlen,
                                        void* smem_ptr) const
    {
        static_assert(std::is_same_v<InOutDataType,
                          ...
                          std::is_same_v<MaskDataType,
                                         remove_cvref_t<typename MaskDramBlockWindowTmp::DataType>>,
                      "wrong!");

        static_assert(kN == B0TDramBlockWindowTmp{}.get_window_lengths()[number<0>{}] &&
                          kM == A0DramBlockWindowTmp{}.get_window_lengths()[number<0>{}] &&
                          kMaxK == B0TDramBlockWindowTmp{}.get_window_lengths()[number<1>{}] &&
        ...
        // A0 tile in LDS
        InOutDataType* a0_lds_ptr = static_cast<InOutDataType*>(static_cast<void*>(smem_ptr));
        auto a0_lds               = make_tensor_view<address_space_enum::lds>(
            a0_lds_ptr, Policy::template MakeA0LdsBlockDescriptor<Problem>());
        auto a0_lds_window =
            make_tile_window(a0_lds, make_tuple(number<kM>{}, number<kMaxK>{}), {0, 0});
        ...
        auto b0t_dram_window = make_tile_window(
            ...
            Policy::template MakeB0TDramTileDistribution<Problem, decltype(gemm)>());
        ...
        auto mask_dram_window = make_tile_window(
            ...
            Policy::template MakeMaskDramTileDistribution<Problem, decltype(gemm)>());
        ...
        do
        {
            ...
            s_acc = gemm(get_slice_tile(a0_lds_window, sequence<0, 0>{}, sequence<kM, kK>{}),
                         get_slice_tile(b0t_tile, sequence<0, 0>{}, sequence<kN, kK>{}));

            static_for<1, k_loops, 1>{}([&](auto i_k) {
                gemm(s_acc,
                     get_slice_tile(
                         a0_lds_window, sequence<0, i_k * kK>{}, sequence<kM, (i_k + 1) * kK>{}),
                     get_slice_tile(
                         b0t_tile, sequence<0, i_k * kK>{}, sequence<kN, (i_k + 1) * kK>{}));
            });
            ...
            sweep_tile_span(s_spans[number<0>{}], [&](auto idx0) {
                sweep_tile_span(s_spans[number<1>{}], [&](auto idx1) {
                    ...
                    s(i_j_idx) = mask[j_idx] ? s[i_j_idx] : -numeric<SMComputeDataType>::infinity();
                });
            });
            ...
            // get the stabilized exp() of the current tile using the latest global maximum value
            sweep_tile_span(s_spans[number<0>{}], [&](auto idx0) {
            ...
            auto rowsum_s = block_tile_reduce<SMComputeDataType>(
                s, sequence<1>{}, f_sum, SMComputeDataType{0}); // rowsum(Pcompute{j})
        ...
        // second main-loop to normalize the gemm result using global max and sum-of-exp
        do
        {
            auto b0t_tile = load_tile(b0t_dram_window);
            auto mask     = load_tile(mask_dram_window);
            ...
            sweep_tile_span(s_spans[number<0>{}], [&](auto idx0) {
                ...
                sweep_tile_span(s_spans[number<1>{}], [&](auto idx1) {
                    constexpr auto i_j_idx = make_tuple(idx0, idx1);
                    if(l[i_idx] == 0.f)
                        s(i_j_idx) = 1.0f / ck_tile::type_convert<SMComputeDataType>(seqlen);
                    else
