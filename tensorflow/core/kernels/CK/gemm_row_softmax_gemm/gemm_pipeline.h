...
struct BlockGemmPipeline {
  ...
  CK_TILE_HOST_DEVICE auto operator()(
      const B1TDramBlockWindowTmp& b1t_dram_block_window_tmp,
    ...
    static_assert(
        kN == B1TDramBlockWindowTmp{}.get_window_lengths()[number<0>{}] &&
            kM == A1DramBlockWindowTmp{}.get_window_lengths()[number<0>{}] &&
            kMaxK ==
                B1TDramBlockWindowTmp{}.get_window_lengths()[number<1>{}] &&
            kMaxK == A1DramBlockWindowTmp{}.get_window_lengths()[number<1>{}],
        "wrong!");

    // A1 tile in LDS
    InOutDataType* a1_lds_ptr =
    ...
    auto a1_lds_window = make_tile_window(
        a1_lds, make_tuple(number<kM>{}, number<kMaxK>{}), {0, 0});
    ...
    s_acc = gemm(
        get_slice_tile(a1_lds_window, sequence<0, 0>{}, sequence<kM, kK>{}),
        get_slice_tile(b1t_tile, sequence<0, 0>{}, sequence<kN, kK>{}));

    static_for<1, k_loops, 1>{}([&](auto i_k) {
      gemm(s_acc,
           get_slice_tile(a1_lds_window, sequence<0, i_k * kK>{},
                          sequence<kM, (i_k + 1) * kK>{}),
           get_slice_tile(b1t_tile, sequence<0, i_k * kK>{},
