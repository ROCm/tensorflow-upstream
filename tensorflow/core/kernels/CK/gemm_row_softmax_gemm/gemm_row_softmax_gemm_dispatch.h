...
template <typename InOutDataType, typename MaskDataType,
          ck_tile::index_t Gemm0MaxK, ck_tile::index_t Gemm1MaxK>
struct gemm_row_softmax_gemm_dispatch {
  using GemmRSGemmProblem_ = ck_tile::GemmRowSoftmaxGemmProblem<
      ...
      MaskDataType, Gemm0RowSoftmaxTileShape<Gemm0MaxK>,
  ...
  static void Run(const GemmRowSoftmaxGemmParams& param, hipStream_t stream) {
    {
      ...
      const bool pad_seqlen =
          !(param.seqlen % Gemm0RowSoftmaxTileShape_::kGemmN == 0);
      const bool pad_head =
          !(param.b0_head_sz % Gemm0RowSoftmaxTileShape_::kMaxK == 0);
      const bool pad_new_head =
          !(param.b1_head_sz % Gemm0RowSoftmaxTileShape_::kGemmM == 0);

      BOOL_SWITCH_3(
          pad_seqlen, kPadSeqLen, pad_head, kPadHeadDim, pad_new_head,
          kPadNewHeadDim, [&] {
            using Gemm0RowSoftmaxTraits_ =
                ck_tile::GemmTileTraits<kPadNewHeadDim, kPadSeqLen, kPadHeadDim,
                                        2>;
...

    {
      ...
      const bool pad_new_head =
          !(param.b1_head_sz % Gemm1TileShape_::kMaxK == 0);

      BOOL_SWITCH_3(
          pad_seqlen, kPadSeqLen, pad_head, kPadHeadDim, pad_new_head,
          kPadNewHeadDim, [&] {
            using Gemm1Traits_ =
                ck_tile::GemmTileTraits<kPadHeadDim, kPadSeqLen, kPadNewHeadDim,
                                        2>;
            ...
            using Gemm1Epilogue =
                ck_tile::Default2DEpilogue<ck_tile::Default2DEpilogueProblem<
                    typename GemmRowSoftmaxGemmTypeConfig<
                        InOutDataType>::GemmAccDataType,
                    InOutDataType, kPadHeadDim, kPadSeqLen>>;

            using Gemm1Kernel =
                ck_tile::GemmKernel<Gemm1Pipeline, Gemm1Epilogue>;
  ...
  static void RunWithGemmRowSoftmaxKernel(const GemmRowSoftmaxGemmParams& param,
                                          hipStream_t stream) {
    const auto kargs = [&] {
      return GemmRowSoftmaxKernel::MakeKargs(
          param.b0_ptr, param.a0_ptr, param.mask_ptr, param.b1_ptr,
          param.num_batch, param.seqlen, param.b0_head_sz, param.b1_head_sz,
          param.b0_batch_stride, param.b0_head_stride, param.a0_ld_sz,
          param.mask_batch_stride, param.b1_batch_stride, param.b1_head_stride);
    }();
  ...
  static void RunWithGemmKernel(const GemmRowSoftmaxGemmParams& param,
                                hipStream_t stream) {
    const auto kargs = [&] {
      return GemmKernel::MakeKargs(
          param.b1_ptr, param.a1_ptr, param.d_ptr, param.num_batch,
          param.seqlen, param.b1_head_sz, param.b0_head_sz,
          param.b1_batch_stride, param.b1_head_stride, param.a1_ld_sz,
          param.d_batch_stride, param.d_head_stride);
    }();
    ...
    (void)ck_tile::launch_kernel(
        ck_tile::stream_config{stream, false},
        ck_tile::make_kernel<kBlockSize.x, kBlockPerCu>(GemmKernel{}, kGridSize,
                                                        kBlockSize, 0, kargs));
  };
};
...
template <typename InOutDataType, typename MaskDataType,
          ck_tile::index_t Gemm0MaxK, ck_tile::index_t Gemm1MaxK>
void run_gemm_row_softmax_gemm(const GemmRowSoftmaxGemmParams& param,
                               hipStream_t stream) {
  gemm_row_softmax_gemm_dispatch<InOutDataType, MaskDataType, Gemm0MaxK,
                                 Gemm1MaxK>::Run(param, stream);
};
