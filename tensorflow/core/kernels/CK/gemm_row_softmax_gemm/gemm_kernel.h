...
struct GemmKernel {
  ...
  using InOutDataType =
      ck_tile::remove_cvref_t<typename GemmPipeline::InOutDataType>;
  using GemmAccDataType =
      ck_tile::remove_cvref_t<typename GemmPipeline::GemmAccDataType>;
  ...
  __host__ static constexpr Kargs MakeKargs(
      const void* b1_ptr, const void* a1_ptr, void* d_ptr,
      ck_tile::index_t num_batch, ck_tile::index_t seqlen,
      ck_tile::index_t b1_head_sz,  // new_head_sz
      ck_tile::index_t d_head_sz,   // head_sz
      ck_tile::index_t b1_batch_stride, ck_tile::index_t b1_head_stride,
      ck_tile::index_t
          a1_ld_sz,  // leading dim size or stride for the non-leading dim
      ck_tile::index_t d_batch_stride, ck_tile::index_t d_head_stride) {
    Kargs kargs{b1_ptr,         a1_ptr,     d_ptr,          num_batch,
                seqlen,         b1_head_sz, d_head_sz,      b1_batch_stride,
                b1_head_stride, a1_ld_sz,   d_batch_stride, d_head_stride};
  ...
  __host__ static constexpr auto GridSize(const Kargs* pKargs) {
    return dim3(
        ck_tile::integer_divide_ceil(pKargs->d_head_sz, GemmPipeline::kM),
  ...
  CK_TILE_DEVICE void operator()(Kargs kargs) const {
    ...
    const index_t i_d_head =
        __builtin_amdgcn_readfirstlane(i_tile_d_head * GemmPipeline::kM);

    const index_t i_seq =
        __builtin_amdgcn_readfirstlane(i_tile_seq * GemmPipeline::kN);

    long_index_t batch_offset_b1 =
        static_cast<long_index_t>(i_batch) * kargs.b1_batch_stride;
    long_index_t batch_offset_d =
    ...
    const InOutDataType* b1_ptr =
        reinterpret_cast<const InOutDataType*>(kargs.b1_ptr) + batch_offset_b1 +
        static_cast<long_index_t>(i_seq);
    const InOutDataType* a1_ptr =
        reinterpret_cast<const InOutDataType*>(kargs.a1_ptr) +
        static_cast<long_index_t>(i_d_head) * kargs.a1_ld_sz;

    InOutDataType* d_ptr =
        reinterpret_cast<InOutDataType*>(kargs.d_ptr) + batch_offset_d +
    ...
    const auto b1t_dram = [&]() {
      const auto b1_dram_naive =
          make_naive_tensor_view<address_space_enum::global>(
              ...
              number<GemmPipeline::kAlignmentB1>{}, number<1>{});
    ...
    const auto a1_dram = [&]() {
      const auto a1_dram_naive =
          make_naive_tensor_view<address_space_enum::global>(
              a1_ptr,
              make_tuple(kargs.d_head_sz,
                         kargs.b1_head_sz),  // [head_sz, new_head_sz]
              make_tuple(kargs.a1_ld_sz, 1),
              number<GemmPipeline::kAlignmentA1>{}, number<1>{});
    ...
    const auto d_dram = [&]() {
      const auto d_dram_naive =
          make_naive_tensor_view<address_space_enum::global>(
              d_ptr, make_tuple(kargs.d_head_sz, kargs.seqlen),
              make_tuple(kargs.d_head_stride, 1),
              number<GemmPipeline::kAlignmentD>{}, number<1>{});
