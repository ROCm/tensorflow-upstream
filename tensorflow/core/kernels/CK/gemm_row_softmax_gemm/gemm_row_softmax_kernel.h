...
// GemmRowSoftmaxGemm problem is solved by two kernels with their pipelines, for
// each batch: S[new_head_sz, seqlen] = A0[new_head_sz, head_sz] @ B0T[seqlen,
// head_sz] B1[new_head_sz, seqlen] = RowSoftmax(S[new_head_sz, seqlen] with
// filtering by mask D[head_sz, seqlen] = A1[head_sz, new_head_sz] @ B1T[seqlen,
// new_head_sz]
...
struct GemmRowSoftmaxKernel {
  using GemmRowSoftmaxPipeline =
      ck_tile::remove_cvref_t<GemmRowSoftmaxPipeline_>;
  static constexpr ck_tile::index_t kBlockSize =
      GemmRowSoftmaxPipeline::kBlockSize;
  static constexpr ck_tile::index_t kBlockPerCu =
      GemmRowSoftmaxPipeline::kBlockPerCu;
  static_assert(kBlockPerCu > 0);

  using InOutDataType =
      ck_tile::remove_cvref_t<typename GemmRowSoftmaxPipeline::InOutDataType>;
  using MaskDataType =
      ck_tile::remove_cvref_t<typename GemmRowSoftmaxPipeline::MaskDataType>;
  ...
  __host__ static constexpr Kargs MakeKargs(
      const void* b0_ptr, const void* a0_ptr, const void* mask_ptr,
      void* b1_ptr, ck_tile::index_t num_batch, ck_tile::index_t seqlen,
      ck_tile::index_t b0_head_sz,  // head_sz
      ck_tile::index_t b1_head_sz,  // new_head_sz
      ck_tile::index_t b0_batch_stride, ck_tile::index_t b0_head_stride,
      ck_tile::index_t
          a0_ld_sz,  // leading dim size or stride for the non-leading dim
      ck_tile::index_t mask_batch_stride, ck_tile::index_t b1_batch_stride,
      ck_tile::index_t b1_head_stride) {
    Kargs kargs{b0_ptr,          a0_ptr,         mask_ptr,   b1_ptr,
                num_batch,       seqlen,         b0_head_sz, b1_head_sz,
                b0_batch_stride, b0_head_stride, a0_ld_sz,   mask_batch_stride,
                b1_batch_stride, b1_head_stride};

    return kargs;
  }

  __host__ static constexpr auto GridSize(const Kargs* pKargs) {
    return dim3(ck_tile::integer_divide_ceil(pKargs->b1_head_sz,
                                             GemmRowSoftmaxPipeline::kM),
                pKargs->num_batch);
  }
  ...
  CK_TILE_DEVICE void operator()(Kargs kargs) const {
    ...
    const index_t i_b1_head = __builtin_amdgcn_readfirstlane(
        i_tile_b1_head * GemmRowSoftmaxPipeline::kM);

    long_index_t batch_offset_b0 =
        static_cast<long_index_t>(i_batch) * kargs.b0_batch_stride;
    long_index_t batch_offset_b1 =
    ...
    const InOutDataType* a0_ptr =
        reinterpret_cast<const InOutDataType*>(kargs.a0_ptr) +
        static_cast<long_index_t>(i_b1_head) * kargs.a0_ld_sz;

    InOutDataType* b1_ptr =
        reinterpret_cast<InOutDataType*>(kargs.b1_ptr) + batch_offset_b1 +
    ...
    const auto b0t_dram = [&]() {
      const auto b0_dram_naive =
          make_naive_tensor_view<address_space_enum::global>(
              b0_ptr, make_tuple(kargs.b0_head_sz, kargs.seqlen),
              make_tuple(kargs.b0_head_stride, 1),
              number<GemmRowSoftmaxPipeline::kAlignmentB0>{}, number<1>{});

      const auto b0t_dram_naive = transform_tensor_view(
          b0_dram_naive,
    ...
    const auto a0_dram = [&]() {
      const auto a0_dram_naive =
          make_naive_tensor_view<address_space_enum::global>(
              a0_ptr,
              make_tuple(kargs.b1_head_sz,
                         kargs.b0_head_sz),  // [new_head_sz, head_sz]
              make_tuple(kargs.a0_ld_sz, 1),
              number<GemmRowSoftmaxPipeline::kAlignmentA0>{}, number<1>{});
    ...
    const auto mask_dram = [&]() {
      const auto mask_dram_naive =
          make_naive_tensor_view<address_space_enum::global>(
              ...
              number<GemmRowSoftmaxPipeline::kAlignmentMask>{}, number<1>{});
    ...
    const auto b1_dram = [&]() {
      const auto b1_dram_naive =
          make_naive_tensor_view<address_space_enum::global>(
              b1_ptr, make_tuple(kargs.b1_head_sz, kargs.seqlen),
              make_tuple(kargs.b1_head_stride, 1),
              number<GemmRowSoftmaxPipeline::kAlignmentB1>{}, number<1>{});
    ...
    auto b0t_dram_window =
        make_tile_window(b0t_dram,
                         make_tuple(number<GemmRowSoftmaxPipeline::kN>{},
                                    number<GemmRowSoftmaxPipeline::kMaxK>{}),
                         {0, 0});

    auto a0_dram_window =
        make_tile_window(a0_dram,
                         make_tuple(number<GemmRowSoftmaxPipeline::kM>{},
                                    number<GemmRowSoftmaxPipeline::kMaxK>{}),
                         {0, 0});

    auto mask_dram_window = make_tile_window(
        mask_dram, make_tuple(number<GemmRowSoftmaxPipeline::kN>{}), {0});
    ...
    GemmRowSoftmaxPipeline{}(b0t_dram_window, a0_dram_window, mask_dram_window,
                             b1_dram_window, kargs.seqlen, smem_ptr);
