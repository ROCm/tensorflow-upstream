...
__global__ void RowReduceKernel(
    ...
    typename std::iterator_traits<T>::value_type initVal) {
  typedef typename std::iterator_traits<T>::value_type value_type;
  const int row = (blockIdx.x * blockDim.x + threadIdx.x) / TF_RED_WARPSIZE;
  ...
  sum = WarpReduce(temp_storage).Reduce(sum, op, min(num_cols, TF_RED_WARPSIZE));
...
__global__ void ColumnReduceMax16ColumnsKernel(
  ...
  if (lane < num_cols) partial_sums[lane * (TF_RED_WARPSIZE+1) + threadIdx.y] = sum;
...
void LaunchScalarReduction(OpKernelContext* ctx, OUT_T out, IN_T in,
  ...
  if (in_size <= 4096) {
    ...
    const int num_threads = 256;
    const int num_blocks = std::min(TF_RED_WARPSIZE, Eigen::divup(in_size, num_threads));
...
void LaunchRowReduction(OpKernelContext* ctx, OUT_T out, IN_T in, int num_rows,
  ...
  gpuprim::TransformInputIterator<int, RowOffset, gpuprim::CountingInputIterator<int>>
